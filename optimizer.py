...
def run_optimization(selected_ids: list[int], n_vehicles: int = 1) -> list[dict]:
    """
    OR-Tools VRP で最短ルートを計算し result.csv に保存する。

    Args:
        selected_ids: 訪問する利用者 ID のリスト（デポ = 0 は自動付加）
        n_vehicles  : 車両数
    ...
    users = df[df["id"].isin(selected_ids)].copy()

    if users.empty:
        pd.DataFrame(columns=["vehicle_id", "order", "user_id", "user"]).to_csv(
